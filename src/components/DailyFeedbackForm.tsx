    ...
    const ScoreSelector = ({
        ...
        return (
            <div className="flex gap-3">
              ...
              ${value === option
                                ? colors[option] + ' scale-[1.02] shadow-sm'
    ...
    const MetricSelector = ({
        ...
        return (
            <div className="space-y-2">
                <div className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-600 chinese-font">
                    {label}
                </div>
                ...
                ${value === score
                                    ? 'bg-slate-100 border-slate-400 text-slate-800 scale-[1.05]'
    ...
    return (
        <div className="bg-stone-50 rounded-2xl p-6 border border-stone-100/50 space-y-6">
            ...
            <div className="space-y-3">
                <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[#5A6A7A]">
                    今日分數 /// OVERALL SCORE
                </div>
                <ScoreSelector
                    value={formData.今日分數}
            ...
            <div className="space-y-3 pt-2">
                <div className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-600 mb-2">
                    今日情緒 // EMOTION METER
                </div>
                <button
                    type="button"
                    onClick={() => setShowEmotionPicker(true)}
                    className="w-full py-3 px-4 rounded-xl border-2 border-slate-200 bg-white text-sm text-slate-600 hover:border-slate-500 hover:shadow-sm transition-all flex items-center justify-center gap-2 font-black"
                >
                    <Sparkles size={14} strokeWidth={2.5} className="text-slate-500" />
                ...
                {formData.情緒.length > 0 && (
                    <div className="text-sm text-slate-800 chinese-font text-center py-2 font-black">
                        {formData.情緒.join(' · ')}
                    </div>
                )}
            </div>
            ...
            <div className="space-y-4 pt-2">
                <div>
                    <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#2D4A31] mb-2 bg-[#E8F0E8] w-fit px-2 py-0.5 rounded">
                    ...
                    <textarea
                        ...
                        className="w-full h-24 px-4 py-3 rounded-xl border-2 border-slate-200 bg-white text-base text-slate-900 chinese-font leading-relaxed resize-none focus:outline-none focus:border-[#8EA68F] transition-colors placeholder:text-slate-300 font-bold"
                    />
                </div>

                <div>
                    <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-700 mb-2 bg-slate-100 w-fit px-2 py-0.5 rounded">
                    ...
                    <textarea
                        ...
                        className="w-full h-24 px-4 py-3 rounded-xl border-2 border-slate-200 bg-white text-base text-slate-900 chinese-font leading-relaxed resize-none focus:outline-none focus:border-slate-400 transition-colors placeholder:text-slate-300 font-bold"
                    />
                </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
                <button
                    ...
                    className="flex-1 py-3 px-4 rounded-xl border-2 border-stone-200 bg-white text-stone-400 font-black text-sm hover:border-slate-300 transition-all"
                >
                    取消
                </button>
                <button
                    ...
                    disabled={isSubmitting}
                    className="flex-1 py-3 px-4 rounded-xl border-2 border-[#8294A5] bg-[#8294A5] text-white font-black text-sm hover:bg-[#6B7F91] transition-all flex items-center justify-center gap-2"
                >
                    {isSubmitting ? (
                        <>
